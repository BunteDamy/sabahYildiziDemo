...
export class RegisterCostsComponent implements OnInit {
  ...
  constructor(
    ...
    this.titleService.setTitle('Maliyetleri Kaydedin');
  }
  ...
  filterGlobal(event: any){
    ...
    this.filterCosts(this.search)
    }

  filterCosts(searchText: string) {
    this.costs = this.allCosts.filter(
        (item) =>
          item.dateCost && item.dateCost.includes(searchText) ||
  ...
  getByIdCost(id: number) {
    this.isViewing = true; 
    this.handlers.headerDialog = 'Maliyeti Görüntüle';
  ...
  dialogEdit(cost: Cost){
    ...
    this.handlers.headerDialog = 'Maliyeti Güncelle'
    this.handlers.handleInsertDialog()
    if(!!cost.id){
      this.costId = cost.id
    }
  ...
  editCost(form: FormGroup) { 
    ...
    this.costsService.updateCost(this.cost, this.costId).subscribe({
      next:() => {
        this.notificationService.showSuccessToast('Maliyet başarıyla güncellendi!')
  ...
  saveNewCost(form: FormGroup){
    if (form.invalid) {
      this.notificationService.showErrorToast('Lütfen gerekli alanları doldurunuz.');
      return;
    }
    this.loadingButton = true;
    this.cost = {
    ...
    this.costsService.postCreateCost(this.cost).subscribe({
      next:() => {
        this.notificationService.showSuccessToast('Maliyetler başarıyla kaydedildi.')
        this.handlers.visibleCreate = false;
        this.loadingButton = false;
        this.getallCosts()
      ...
      error: (error) => {
        const errorMessage = error?.error ?? 'İşlem sırasında bir hata oluştu. Lütfen daha sonra tekrar deneyin.';
        this.notificationService.showErrorToast(errorMessage)
        this.loadingButton = false;
        this.getallCosts()
      }
  ...
  openCreate() {
    ...
    this.handlers.headerDialog = 'Maliyeti Kaydet'
