...
export class CalculateComponent implements OnInit {
  ...
  constructor(
    ...
    this.titleService.setTitle('Birim maliyetleri hesaplayın');
      ...
      next: (response) => {
        if (response.isSuccess && response.data) {
          ...
          if (productTotalCost) {
            ...
            this.notificationService.showErrorToast('Ürün maliyeti bulunamadı.');
          }
        }
      },
      error: () => {
        this.notificationService.showErrorToast('Düzenleme için veri yüklenirken hata oluştu.');
      },
    });
  }
  ...
  getAllProducts(){
    this.productService.getAllProducts().subscribe({
      ...
      error: () => {
        this.notificationService.showErrorToast('Ürün verileri yüklenirken hata oluştu');
      }
    })
  }
  ...
  newProductCost() {
    ...
    this.productCostService.postCreateProductCost(payload).subscribe({
      next: () => {
        ...
        this.notificationService.showSuccessToast('Hesaplama başarıyla kaydedildi!')
        this.visibleDialogProduct = false;
        this.clearForm();
      },
      error: () => {
        this.notificationService.showErrorToast('Kayıt verileri gönderilirken hata oluştu');
        this.loadingButton = false;
      }
    });
  }
